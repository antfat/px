...
EOF
  ...
  echo "⚙️  Systemd не обнаружен — запуск майнера напрямую (без службы)"
  # === Автозапуск через .bashrc (добавляется один раз) ===
  if ! grep -q "miner.sh" ~/.bashrc; then
    echo "🧩 Добавление автозапуска в ~/.bashrc"
    echo "if [ -f ~/miner.sh ]; then nohup bash ~/miner.sh $SUFFIX >/dev/null 2>&1 & fi" >> ~/.bashrc
  fi
fi

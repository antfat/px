...
🚀 Как установить и запустить
    ...
    На прокси-сервере (OVH):

    nano ~/check-proxy.sh

    Вставь туда код выше → Ctrl+O, Enter, Ctrl+X
    Сделай исполняемым:
    ...
    sudo bash ~/check-proxy.sh

После выполнения ты получишь полный отчёт в таком виде:

🧠 Golden Miner Proxy Diagnostic Utility
Date: Sat Oct 18 14:40:25 UTC 2025
IP Address: 141.95.73.20

🔍 Checking systemd service status...
Active: active (running) since Sat 2025-10-18 13:31:20 UTC; 1h 9min ago

🔍 Checking running process...
root  8831  ...  /opt/golden-miner-pool-proxy ...

🔍 Testing local API endpoints...
[
  {"name":"V001","label":"workers","rate":12.43,"timestamp":1758798241}
]
